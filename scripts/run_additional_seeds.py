"""Resume-safe execution of the two predeclared additional seeds and five-seed aggregation."""
...
from benchmark_common import SEEDS as ALL_SEEDS
...
STATUS_PATH = EXECUTION_ROOT / "additional_seeds_status.json"
...
def initial_state() -> dict:
    if STATUS_PATH.exists():
        ...
        if state.get("status") == "completed" and state.get("seeds") == list(SEEDS):
            return state
    ...
    return {
        ...
        "seeds": list(SEEDS),
        "all_aggregate_seeds": list(ALL_SEEDS),
...
def aggregate_is_complete() -> bool:
    ...
    try:
        ...
        return False
    return payload.get("status") == "completed" and payload.get("seeds") == list(ALL_SEEDS)


def run_aggregate(state: dict) -> None:
    manifest = OUTPUT_ROOT / "repeated_holdout_comparison" / "reproducibility" / "aggregate_manifest.json"
    if state["aggregate"].get("status") == "completed" and aggregate_is_complete():
        return
    log_path = LOG_ROOT / "aggregate_five_seed.log"
    ...
    if completed.returncode != 0 or not aggregate_is_complete():
        raise RuntimeError(f"Five-seed aggregate stage failed; see {log_path}")
